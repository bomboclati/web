...
"""
    ...
    async def _parse_and_handle_response(self, session, provider, provider_url, payload, messages, resp) -> dict:
        ...
        if self._validate_json_response(res_json):
            # Ensure 'reasoning' and 'walkthrough' are present - safely handle missing keys
            if "reasoning" not in res_json or res_json.get("reasoning") in [None, "", "Raw text response", "Standard response"]:
                res_json["reasoning"] = "Thinking..."
            
            if "walkthrough" not in res_json or not res_json.get("walkthrough"):
                res_json["walkthrough"] = "Planning..."

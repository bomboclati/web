...
class MiroBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        from modules.auto_setup import VerifyButton, AcceptRulesButton, CreateTicketButton, SuggestionButton, ApplyStaffButton, RoleSelectButton, AutoSetupView
        ...
        self.add_view(AutoSetupView(self, 0))
...
async def on_member_join(member: discord.Member):
    ...
    try:
        await bot.guardian.on_member_join(member)
    except Exception as e:
        logger.warning(f"Guardian on_member_join error: {e}")
    # Autorole: assign configured role to new members
    try:
        autorole_id = dm.get_guild_data(member.guild.id, "autorole_id")
        if autorole_id:
            role = member.guild.get_role(int(autorole_id))
            if role:
                await member.add_roles(role, reason="Autorole on join")
    except Exception as e:
        logger.warning(f"Autorole on_member_join error: {e}")
...
async def on_raw_reaction_add(payload):
    """Handle reaction add for reaction roles and staff extras"""
    if payload.user_id == bot.user.id:
        return
    # Handle reaction roles (from action_create_reaction_role)
    try:
        guild = bot.get_guild(payload.guild_id)
        if guild:
            rr_config = dm.get_guild_data(guild.id, "reaction_roles_v2", {})
            msg_config = rr_config.get(str(payload.message_id))
            if msg_config:
                emoji_str = str(payload.emoji)
                role_id = msg_config.get("roles", {}).get(emoji_str)
                if role_id:

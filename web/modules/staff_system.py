...
class StaffReviewPersistentView(ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: ui.Button):
        ...
        # --- Assign staff role if configured ---
        if applicant:
            staff_role_id = dm.get_guild_data(guild.id, "staff_role_id") or dm.get_guild_data(guild.id, "staff_log_channel_id")
            # Try to find role by stored ID first, then by name
            staff_role = None
            stored_role_id = dm.get_guild_data(guild.id, "staff_role_id")
            if stored_role_id:
                staff_role = guild.get_role(int(stored_role_id))
            if not staff_role:
                staff_role = discord.utils.get(guild.roles, name="Staff") or \
                             discord.utils.get(guild.roles, name="Moderator") or \
                             discord.utils.get(guild.roles, name="Team")
            if staff_role:
                try:
                    await applicant.add_roles(staff_role, reason=f"Staff application approved by {interaction.user}")
                    logger.info("Assigned staff role %s to %s", staff_role.name, applicant)
                except discord.Forbidden:
                    logger.warning("Missing permission to assign staff role to %s", applicant)
                except Exception as e:
                    logger.error("Error assigning staff role: %s", e)

            try:
                await applicant.send(f"✅ Your staff application for **{guild.name}** has been **approved**! Welcome to the team!")
        ...
        await interaction.followup.send("✅ Application approved and role assigned!", ephemeral=True)

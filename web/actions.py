...
class ActionHandler:
    ...
    async def action_assign_role(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        ...
        if not isinstance(role_ids, list):
            if role_ids is not None:  # Only add non-None values
                role_ids = [role_ids]
            else:
                role_ids = []
        if "role_id" in params and params["role_id"] is not None:
            role_ids.append(params["role_id"])

        role_names = params.get("role_names", [])
        if not isinstance(role_names, list):
            if role_names is not None:  # Only add non-None values
                role_names = [role_names]
            else:
                role_names = []
        if "role_name" in params and params["role_name"] is not None:
            role_names.append(params["role_name"])
        if "name" in params and params["name"] is not None:
            role_names.append(params["name"])
        ...
        if not isinstance(user_ids, list):
            if user_ids is not None:  # Only add non-None values
                user_ids = [user_ids]
            else:
                user_ids = []
        ...
        if "user" in params and params["user"] is not None:
            user_ids.append(params["user"])

        usernames = params.get("usernames", [])
        if not isinstance(usernames, list):
            if usernames is not None:  # Only add non-None values
                usernames = [usernames]
            else:
                usernames = []
        ...
        if "user_name" in params and params["user_name"] is not None:
            usernames.append(params["user_name"])
    ...
    async def action_setup_leveling(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        ...
        from modules.auto_setup import AutoSetup
        setup = AutoSetup(self.bot)
        result = await setup._setup_leveling(interaction.guild)
        return bool(result), {"action": "undo_leveling", "guild_id": interaction.guild.id}
    ...
    async def action_set_server_icon(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as resp:
                    if resp.status != 200:
                        return False, {"error": f"Failed to fetch image: HTTP {resp.status}"}
                    data = await resp.read()
            await interaction.guild.edit(icon=data)
            return True, {"action": "server_icon_set"}
        except Exception as e:
            logger.error(f"Error setting server icon: {e}")
            return False, {"error": str(e)}

    async def action_set_server_banner(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Sets the server banner from a URL (requires boost level 2)."""
        url = params.get("url")
        if not url:
            return False, {"error": "Missing url"}
        try:
            ...
            return True, {"action": "server_banner_set"}
        except Exception as e:
            logger.error(f"Error setting server banner: {e}")
            return False, {"error": str(e)}

    async def action_set_server_name(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Renames the server."""
        name = params.get("name")
        if not name:
    ...
    async def action_set_server_description(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Sets the server description."""
        description = params.get("description", "")
        try:
            await interaction.guild.edit(description=description)
            return True, {"description": description}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_bulk_delete_messages(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Bulk deletes messages from a channel (up to 100, max 14 days old)."""
        channel_name = params.get("channel") or params.get("channel_name")
        ...
        try:
            ...
            return True, {"deleted": len(deleted)}
        except discord.Forbidden:
            return False, {"error": "Missing manage_messages permission"}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_mass_assign_role(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Assigns a role to all members (or all without a given role)."""
        role_name = params.get("role_name")
        require_no_role = params.get("require_no_role")
        if not role_name:
            return False, {"error": "Missing role_name"}
        guild = interaction.guild
        role = discord.utils.get(guild.roles, name=role_name)
        if not role:
            return False, {"error": f"Role '{role_name}' not found"}
        filter_role = discord.utils.get(guild.roles, name=require_no_role) if require_no_role else None
        count = 0
        for member in guild.members:
            if member.bot:
                continue
            if filter_role and filter_role in member.roles:
                continue
            if role not in member.roles:
                try:
                    await member.add_roles(role, reason="AI mass_assign_role")
                    count += 1
                except Exception:
                    pass
        return True, {"assigned": count, "role": role_name}

    async def action_mass_remove_role(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        ...
        for member in role.members:
            try:
                await member.remove_roles(role, reason="AI mass_remove_role")
                count += 1
            except Exception:
                pass
        return True, {"removed": count, "role": role_name}

    async def action_create_giveaway(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Creates a giveaway embed with reaction entry."""
        channel_name = params.get("channel") or params.get("channel_name")
        prize = params.get("prize", "Mystery Prize")
        duration_minutes = int(params.get("duration_minutes", 60))
        winners = int(params.get("winners", 1))
        channel = discord.utils.get(interaction.guild.channels, name=channel_name) if channel_name else interaction.channel
        if not channel:
            return False, {"error": "Channel not found"}
        import datetime as _dt
        end_time = _dt.datetime.now(_dt.timezone.utc) + _dt.timedelta(minutes=duration_minutes)
        embed = discord.Embed(
            title="🎉 GIVEAWAY 🎉",
        ...
        # Store giveaway data
        giveaways = dm.get_guild_data(interaction.guild.id, "giveaways", {})
        giveaways[str(msg.id)] = {
            "channel_id": channel.id, "prize": prize,
            "winners": winners, "end_time": end_time.isoformat(),
            "message_id": msg.id
        }
        dm.update_guild_data(interaction.guild.id, "giveaways", giveaways)
        return True, {"message_id": msg.id, "prize": prize}

    async def action_send_embed(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Sends a richly-formatted embed to a channel."""
        channel_name = params.get("channel") or params.get("channel_name")
        title = params.get("title", "")
        description = params.get("description", "")
        color = parse_color(params.get("color", "blue"))
        fields = params.get("fields", [])
        footer = params.get("footer", "")
        ...
        try:
            msg = await channel.send(embed=embed)
            return True, {"message_id": msg.id, "channel_id": channel.id}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_create_reaction_role(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Creates a reaction-role message: react with emoji → get role."""
        channel_name = params.get("channel") or params.get("channel_name")
        title = params.get("title", "🎭 Role Selection")
        roles = params.get("roles", [])  # [{emoji, role_name, description}]
        channel = discord.utils.get(interaction.guild.channels, name=channel_name) if channel_name else interaction.channel
        if not channel:
            return False, {"error": "Channel not found"}
        guild = interaction.guild
        ...
        for entry in roles:
            ...
            desc = entry.get("description", role_name)
            role = discord.utils.get(guild.roles, name=role_name)
            if role:
                description_lines.append(f"{emoji} → **{role_name}** — {desc}")
                role_map[emoji] = role.id
        embed = discord.Embed(title=title, description="\n".join(description_lines) or "No roles configured", color=discord.Color.blue())
        msg = await channel.send(embed=embed)
        for emoji in role_map:
            try:
                ...
                pass
        # Store reaction roles config
        rr = dm.get_guild_data(guild.id, "reaction_roles_v2", {})
        rr[str(msg.id)] = {"channel_id": channel.id, "roles": role_map}
        dm.update_guild_data(guild.id, "reaction_roles_v2", rr)
        return True, {"message_id": msg.id, "roles_count": len(role_map)}

    async def action_move_member_to_voice(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        ...
        channel = discord.utils.get(interaction.guild.voice_channels, name=channel_name) if channel_name else None
        if not channel:
            return False, {"error": "Voice channel not found"}
        try:
            await member.move_to(channel)
            return True, {"user": member.display_name, "channel": channel.name}
        except discord.Forbidden:
            return False, {"error": "Missing move_members permission"}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_disconnect_member_from_voice(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            return True, {"user": member.display_name}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_set_slowmode_all(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Sets slowmode on all text channels in the server."""
        ...
        for channel in interaction.guild.text_channels:
            try:
                ...
                pass
        return True, {"channels_updated": count, "delay": delay}

    async def action_create_webhook(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Creates a webhook in a channel."""
        channel_name = params.get("channel") or params.get("channel_name")
        name = params.get("name", "Bot Webhook")
        channel = discord.utils.get(interaction.guild.channels, name=channel_name) if channel_name else interaction.channel
        if not channel:
            return False, {"error": "Channel not found"}
        try:
            wh = await channel.create_webhook(name=name)
            dm.update_guild_data(interaction.guild.id, f"webhook_{name}", {"url": wh.url, "id": wh.id})
            return True, {"webhook_url": wh.url, "name": name}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_send_via_webhook(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Sends a message through a webhook (supports custom username/avatar)."""
        ...
        embed_data = params.get("embed")
        if not webhook_url:
            return False, {"error": "Missing webhook_url"}
        try:
            import aiohttp
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(webhook_url, json=payload) as resp:
                    if resp.status not in (200, 204):
                        return False, {"error": f"Webhook returned HTTP {resp.status}"}
            return True, {"sent": True}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_set_verification_level(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Changes the server verification level."""
        level_str = params.get("level", "medium").lower()
        level_map = {
        ...
        level = level_map.get(level_str, discord.VerificationLevel.medium)
        try:
            await interaction.guild.edit(verification_level=level)
            return True, {"level": level_str}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_unban_user(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Unbans a user by ID or username."""
        user_id = params.get("user_id")
        username = params.get("username")
        if not user_id and username:
            async for ban_entry in interaction.guild.bans():
                if ban_entry.user.name.lower() == username.lower():
                    ...
                    break
        if not user_id:
            return False, {"error": "Could not find banned user"}
        try:
            await interaction.guild.unban(discord.Object(id=int(user_id)), reason="AI action unban")
            return True, {"user_id": user_id}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_create_private_channel(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Creates a private channel visible only to specified roles."""
        name = params.get("name", "private-channel")
        allowed_roles = params.get("allowed_roles", [])
        category_name = params.get("category")
        guild = interaction.guild
    ...
    async def action_archive_channel(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Archives a channel by making it read-only and moving to an archive category."""
        channel_name = params.get("channel_name") or params.get("channel")
        archive_category_name = params.get("archive_category", "📦 Archive")
        guild = interaction.guild
        channel = discord.utils.get(guild.channels, name=channel_name) if channel_name else interaction.channel
        if not channel:
            return False, {"error": "Channel not found"}
        archive_cat = discord.utils.get(guild.categories, name=archive_category_name)
        if not archive_cat:
            archive_cat = await guild.create_category(archive_category_name)
        try:
            await channel.edit(
                category=archive_cat,
                overwrites={guild.default_role: discord.PermissionOverwrite(send_messages=False, view_channel=True)}
            )
            return True, {"channel": channel.name, "archived_to": archive_category_name}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_sync_channel_permissions(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
    ...
    async def action_create_modmail_thread(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Creates a modmail thread for a user."""
        user_id = params.get("user_id")
        username = params.get("username")
        ...
        modmail_ch = interaction.guild.get_channel(modmail_ch_id) if modmail_ch_id else None
        if not modmail_ch:
            return False, {"error": "Modmail channel not configured"}
        embed = discord.Embed(title=f"📬 Modmail Thread: {member.display_name}", description=message, color=discord.Color.blurple())
        embed.set_footer(text=f"User ID: {member.id}")
        thread_msg = await modmail_ch.send(embed=embed)
        try:
            thread = await thread_msg.create_thread(name=f"modmail-{member.name}")
        except Exception:
            thread = None
        try:
            await member.send(f"📬 **{interaction.guild.name}** staff has opened a modmail thread with you: {message}")
        except Exception:
            pass
        return True, {"thread": thread.name if thread else "N/A", "user": member.display_name}

    async def action_add_bot_to_channel(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Grants the bot explicit access to view/send in a channel."""
        channel_name = params.get("channel_name") or params.get("channel")
        channel = discord.utils.get(interaction.guild.channels, name=channel_name) if channel_name else interaction.channel
        if not channel:
            return False, {"error": "Channel not found"}
        ...
        try:
            await channel.set_permissions(bot_member, view_channel=True, send_messages=True, read_message_history=True)
            return True, {"channel": channel.name}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_query_bans(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Lists banned users in the server."""
        limit = int(params.get("limit", 10))
        bans = []
        async for ban_entry in interaction.guild.bans(limit=limit):
            bans.append({"user": str(ban_entry.user), "reason": ban_entry.reason or "No reason"})
        text = "\n".join([f"• **{b['user']}** — {b['reason']}" for b in bans]) or "No bans found"
        await self._send_query_result(interaction, "🔨 Banned Users", text)
        return True, {"bans": bans}

    async def action_setup_starboard(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Sets up a starboard channel."""
        guild = interaction.guild
        channel_name = params.get("channel", "⭐-starboard")
        threshold = int(params.get("threshold", 3))
        channel = discord.utils.get(guild.text_channels, name=channel_name)
        if not channel:
            channel = await guild.create_text_channel(channel_name)
        dm.update_guild_data(guild.id, "starboard_channel", channel.id)
        dm.update_guild_data(guild.id, "starboard_threshold", threshold)
        embed = discord.Embed(title="⭐ Starboard Configured", description=f"Messages with {threshold}+ ⭐ reactions will appear here!", color=discord.Color.gold())
        await channel.send(embed=embed)
        return True, {"channel": channel.name, "threshold": threshold}

    async def action_setup_counting_channel(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Creates a counting game channel."""
        guild = interaction.guild
        channel_name = params.get("channel", "🔢-counting")
        channel = discord.utils.get(guild.text_channels, name=channel_name)
        if not channel:
            channel = await guild.create_text_channel(channel_name, topic="Count to infinity! Next number: 1")
        dm.update_guild_data(guild.id, "counting_channel", channel.id)
        dm.update_guild_data(guild.id, "counting_next", 1)
        await channel.send("🔢 Counting game started! Type **1** to begin!")
        return True, {"channel": channel.name}

    async def action_create_role_with_permissions(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Alias that creates role with full permission spec (routed to existing handler)."""
        return await self.action_create_role(interaction, params)

    async def action_set_afk_channel(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Sets the AFK voice channel and timeout."""
        channel_name = params.get("channel")
        timeout = int(params.get("timeout", 300))
        channel = discord.utils.get(interaction.guild.voice_channels, name=channel_name) if channel_name else None
        try:
            await interaction.guild.edit(afk_channel=channel, afk_timeout=timeout)
            return True, {"channel": channel.name if channel else None, "timeout": timeout}
        except Exception as e:
            return False, {"error": str(e)}

    async def action_send_rules_embed(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Sends a formatted server rules embed."""
        channel_name = params.get("channel") or params.get("channel_name")
        rules = params.get("rules", [])
        title = params.get("title", "📜 Server Rules")
        color = parse_color(params.get("color", "red"))
        channel = discord.utils.get(interaction.guild.channels, name=channel_name) if channel_name else interaction.channel
        if not channel:
            channel = interaction.channel
        ...
        for i, rule in enumerate(rules, 1):
            if isinstance(rule, dict):
                embed.add_field(name=f"Rule {i}: {rule.get('title', '')}", value=rule.get("description", ""), inline=False)
            else:
                embed.add_field(name=f"Rule {i}", value=str(rule), inline=False)
        embed.set_footer(text="Breaking rules may result in moderation action.")
        msg = await channel.send(embed=embed)
        return True, {"message_id": msg.id}

    async def action_promote_channel_to_announcement(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Converts a text channel to an announcement channel."""
        channel_name = params.get("channel_name") or params.get("channel")
        channel = discord.utils.get(interaction.guild.text_channels, name=channel_name) if channel_name else interaction.channel
        if not channel:
            return False, {"error": "Channel not found"}
        try:
    ...
    async def action_setup_autorole(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        """Configures a role to be automatically given to new members."""
        role_name = params.get("role_name")
        if not role_name:
            return False, {"error": "Missing role_name"}
        guild = interaction.guild
        role = discord.utils.get(guild.roles, name=role_name)
        if not role:
            role = await guild.create_role(name=role_name, reason="AI autorole setup")
        dm.update_guild_data(guild.id, "autorole_id", role.id)
        return True, {"role": role_name, "role_id": role.id}

    async def action_list_members_with_role(self, interaction: discord.Interaction, params: Dict[str, Any]) -> Tuple[bool, Optional[Dict]]:
        ...
        text = "\n".join([f"• {m}" for m in members[:30]]) or "No members with this role"
        await self._send_query_result(interaction, f"👥 Members with {role_name}", text, [{"name": "Count", "value": str(len(members)), "inline": True}])
